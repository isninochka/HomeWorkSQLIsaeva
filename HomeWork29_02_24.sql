...
5-Найти сотрудников, занимающих позицию "Manager" и "HR", и отобразить только их имена, ограничив выборку первыми двумя.
  db.employees.find({position:{$in:['Manager','HR']}},{name:1}).limit(2)
...
7-Найти сотрудников с зарплатой выше 5000, пропустив первого, и отобразить только их имена.
  db.employees.find({salary:{$gt:5000}},{name:1}).skip(1)
...
9-Найти сотрудников в возрасте от 25 до 35 лет и ограничить список первыми тремя.
  db.employees.find({age:{$gt:25,$lt:35}}).limit(3)
...
13-Найти сотрудников с позицией "Analyst" и вернуть только их имена и позиции, отсортированных по возрасту в порядке убывания, пропустив первого.
  db.employees.find({position:{$eq:'Analyst'}},{name:1,position:1}).sort({age:-1}).skip(1)
...
14-Получить первых двух сотрудников из департамента HR.
  db.departments.aggregate([{$match:{name:"HR"}},{$project:{employees:{$slice:["$employees",2]}}}])
  
15-Получить последних трех сотрудников из департамента IT.
  db.departments.aggregate([{$match:{name:"IT"}},{$project:{employees:{$slice:["$employees",-3]}}}])
  
16-Получить третьего и четвертого сотрудников из департамента HR.
  db.departments.aggregate([{$match:{name:"HR"}},{$project:{employees:{$slice:["$employees",2,2]}}}])
  
17-Получить первого сотрудника из департамента IT и отобразить только его имя.
  db.departments.aggregate([{$match:{name:"IT"}},{$project:{_id: 0,"employees.name":1}},{$unwind:"$employees"},{$limit:1}])
